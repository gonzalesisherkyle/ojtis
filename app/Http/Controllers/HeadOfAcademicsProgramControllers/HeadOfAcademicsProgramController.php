...
namespace App\Http\Controllers\HeadOfAcademicsProgramControllers;
...
class HeadOfAcademicsProgramController extends Controller
{
    ...
    public function index()
    {
        ...
        ->where('tbl_files.adviser_approval','=','approved')
        ->where('tbl_files.category_id',8 )
        ->get();

        return view('head-of-academics-program.files.pending-letter', compact('letters', 'user'));
    }
    ...
    public function account()
    {
        ...
        return view('head-of-academics-program.account', compact('user'));
    }
    ...
    public function approvedLetter()
    {
        ...
        return view('head-of-academics-program.files.approved-letter', compact('letters', 'user'));
    }
    ...
    public function disapprovedLetter()
    {
        ...
        $letters = Files::join('users','users.user_id' ,'=','tbl_files.uploaded_by')
        ->where('tbl_files.headAcademics_approval','=','disapproved')
        ->where('tbl_files.category_id',8 )
        ->get();

        return view('head-of-academics-program.files.disapproved-letter', compact('letters', 'user'));
    }
    ...
    public function clearRL($id)
    {
        ...
        $users = Auth::user();
        $email = User::where('applying_as', 'Director')
        ...
        if($rLetter)
        {
            ...
            $rLetter = Files::where('file_id', $id)->update([
                'headAcademics_approval' => 'approved'
            ]);

            Mail::send(new ApprovalRL($users, $email));
        }
        else
        {
    ...
    public function rejectRL($id)
    {
        ...
        if($rLetter)
        {
            ...
            $rLetter = Files::where('file_id', $id)->update([
                'headAcademics_approval' => 'disapproved'
            ]);
        }
        else
        {
